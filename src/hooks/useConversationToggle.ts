...
export function useConversationToggle() {
  ...
  const toSpecifiedConversation = useCallback(
    async (params: ToSpecifiedConversationParams) => {
      const { sourceID, sessionType, isJump, isChildWindow } = params;
      if (window.electronAPI?.enableCLib && isChildWindow) {
        emitToSpecifiedWindow("JUMP_TO_SPECIFIED_CONVERSATION", params);
        return;
      }
