...
export const PersonalSettingsContent = ({
  ...
  const updateSelfInfoWithMainWindow = (info: Partial<BusinessUserInfo>) => {
    if (window.electronAPI) {
      emitToSpecifiedWindow("CALL_STORE_FUNCTION", {
        store: "user",
  ...
  const deregisterAccount = () => {
    modal.confirm({
      title: t("toast.deregisterAccount"),
      content: (
        <div className="flex items-end">
          <div>{t("toast.confirmDeregisterAccount")}</div>
          <div className="ml-1 text-xs text-[var(--sub-text)]">
            {t("toast.deregisterDescription")}
          </div>
        </div>
      ),
      onOk: async () => {
    ...
    <div className="flex flex-col bg-[var(--chat-bubble)]">
      ...
      <div className="flex-1 overflow-y-auto">
            ...
            <div className="pb-8 pl-1">
              ...
              <div>
                <Spin spinning={addFriendPermissionUpdating}>
                  ...
                  >
                    {t("placeholder.notVerifyAddFriend")}
                  </Checkbox>
                </Spin>
              </div>
            </div>
        ...
        <div className="cursor-pointer px-6 py-4">
          <div
            className="text-base font-medium text-[var(--warn-text)]"
            onClick={deregisterAccount}
          >
            {t("toast.deregisterAccount")}
          </div>
        </div>
      </div>
    </div>
