...
import { BusinessUserInfo, RegisterType, searchBusinessUserInfo } from "@/api/login";
  ...
  const searchData = async () => {
    ...
    if (isSearchGroup) {
      try {
        ...
        const groupInfo = data.find((group) => group.status !== GroupStatus.Dismissed);
        setLoading(false);
        if (!groupInfo) {
      ...
      try {
        ...
        setLoading(false);
        if (!total || !checkUser(users[0], keyword)) {
          message.warning(t("empty.noSearchResults"));
          return;
        }
