...
const CustomTabPane = ({ closeAllPop }: { closeAllPop?: () => void }) => {
  ...
  const deleteCustomEmoji = useCustomEmojiStore((state) => state.deleteCustomEmoji);
  ...
  const fileHandle = async (options: UploadRequestOption) => {
    ...
    await addCustomEmoji({ url, width, height, path: file.path ?? "" });
  };
  ...
  const customEmojisLength = Math.max(
    18 - 1,
    Math.ceil((customEmojis.length + 1) / 6) * 6 - 1,
  );
  ...
  return (
    <div className="my-5 grid flex-1 grid-cols-6 gap-3 overflow-auto px-5">
      ...
      {Array.from({ length: customEmojisLength }, (_, idx) => (
        <CustomEmojiItemEl
          key={idx}
          idx={idx}
...
const EmojiPopContent = ({
  ...
  useEffect(() => {
    ...
    const emojiClickHander = (event: EmojiClickEvent) => {
      sendEmoji?.(event.detail.unicode!);
      closeAllPop?.();
    };
    // @ts-ignore
