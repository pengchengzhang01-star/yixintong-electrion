...
import { escapeHtml, generateAvatar, secondsToTime } from "./common";
import {
  AtTextElem,
  ConversationItem,
  MessageItem,
      ...
      case MessageType.MsgPinned:
        ...
        return t(
          pinMessageDetail.pinned
            ? "messageDescription.pinnedMessage"
            : "messageDescription.cancelPinnedMessage",
          {
            operator: linkWrap({
    ...
    switch (message.contentType) {
      ...
      case MessageType.CustomMessage:
        ...
        if (customData.customType === CustomMessageType.MassMsg)
          return t("messageDescription.massMessage");
        if (customData.customType === CustomMessageType.Call)
          return t("messageDescription.callMessage");
      ...
      case MessageType.MarkdownMessage:
        return message.markdownTextElem?.content ?? "";
...
export const initStore = () => {
  ...
  IMSDK.getSignalingInvitationInfoStartApp().then(async ({ data }) => {
    if (!data.invitation) {
      return;
    }
    if (data.invitation.groupID) {
      const { data: roomData } = await IMSDK.signalingGetRoomByGroupID(
        data.invitation!.groupID,
      );
      emit("OPEN_RTC_MODAL", {
      ...
      const { data: userInfos } = await IMSDK.getUsersInfo([
        data.invitation.inviterUserID,
      ]);
      emit("OPEN_RTC_MODAL", {
        invitation: data.invitation,
        participant: {
...
export const parseAtToModel = (atel: AtTextElem) => {
  ...
  arr?.map((match) => {
    ...
    if (member) {
      str = str.replace(
        match,
        `<span class="im-mention-blot" data-id="${member.atUserID}" data-value="${member.groupNickname}" contenteditable="false"><span contenteditable="false">@${member.groupNickname}</span></span>${mentionCursorChar}`,
      );
    }
  });
  return str;
...
export const createRTCNotification = async ({
  ...
  if (!window.Notification || window.Notification.permission !== "granted") return;
  const notification = new Notification(t("placeholder.meetingInvitation"), {
    dir: "auto",
    tag: uuidv4(),
    renotify: true,
    body: message,
